...
void
Texture::loadBMP(std::string_view path, GLint texMode)
{
    if (this->id != 0)
    {
    ...
    if (format == GL_RGBA)
        flipcpyBGRAtoRGBA((u8*)pixels.data(), (u8*)&bmp[bmp.start], width, height);
    else
        flipcpyBGRtoRGB((u8*)pixels.data(), (u8*)&bmp[bmp.start], width, height);
